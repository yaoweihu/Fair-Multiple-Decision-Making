...
def run_unconstrained_model(**kwargs):
    ...
    clf.fit1(s_train, x1_train, y1_train, dummy_cols=['x1'], enable_s=False)
    y1_train_pred = clf.predict1(s_train, x1_train, dummy_cols=['x1'], enable_s=False)
    ...
    train_data_new = generate_eval_data(s_train, x1_train, y1_train, **kwargs)

    clf.fit2(s_train, x12_train, y2_train, dummy_cols=['x1'], enable_s=False)
    y2_train_pred = clf.predict2(s_train, x12_train, dummy_cols=['x1'], enable_s=False)
    ...
    y1_test_pred = clf.predict1(s_test, x1_test, dummy_cols=['x1'], enable_s=False)
    res_y1_test_pred = clf.eval(y1_test, y1_test_pred, s_test)

    # -------------- test 2 ---------------
    test_data_new = generate_eval_data(s_test, x1_test, y1_test_pred, **kwargs)
    x2_test = test_data_new[x2_name]
    x12_test = pd.concat([x1_test, y1_test_pred, x2_test], axis=1)
    y2_test = test_data_new[y2_name]

    y2_test_pred = clf.predict2(s_test, x12_test, dummy_cols=['x1'], enable_s=False)
...
def run_separate_model(**kwargs):
    ...
    clf.fit1(s_train, x1_train, y1_train, s_pos_num=s_pos_num, s_neg_num=s_neg_num, s_pos_val=1, dummy_cols=['x1'], enable_s=False)
    y1_train_pred = clf.predict1(s_train, x1_train, dummy_cols=['x1'], enable_s=False)
    res_y1_train_pred = clf.eval(y1_train, y1_train_pred, s_train)
    ...
    clf.fit2(s_train, x12_train, y2_train, s_pos_num=s_pos_num, s_neg_num=s_neg_num, s_pos_val=1, dummy_cols=['x1'], enable_s=False)
    y2_train_pred = clf.predict2(s_train, x12_train, dummy_cols=['x1'], enable_s=False)
    res_y2_train_pred = clf.eval(y2_train, y2_train_pred, s_train)
...
def run_serial_model(**kwargs):
    ...
    # -------------- train 2 ---------------
    train_data_new = generate_eval_data(s_train, x1_train, y1_train_pred, **kwargs)
    y1_train = train_data_new[y1_name]
...
def run_joint_model(**kwargs):
    ...
    res_train = clf.fit(s_train, x1_train, x12_train, dummy_cols=['x1'], enable_s=False)
