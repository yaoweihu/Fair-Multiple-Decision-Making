...
def adult():
    adult_df = pd.read_csv(os.path.join(src_path, '../data/adult.csv'))
    ...
    adult_df['hours'] = adult_df['hours-per-week'].apply(lambda a: 1 if a > 41 else 0)
    ...
    X_resampled, y_resampled = ros.fit_sample(adult_df[['age', 'sex', 'education', 'marital-status', 'hours', 'income']], adult_df['workclass'])
    adult_df = pd.DataFrame(data=np.hstack([X_resampled, y_resampled.values.reshape((y_resampled.__len__(), 1))]),
                            columns=['age', 'sex', 'education', 'marital-status', 'hours', 'income', 'workclass'])
    X_resampled, y_resampled = ros.fit_sample(adult_df[['age', 'sex', 'workclass', 'education', 'marital-status', 'hours']], adult_df['income'])
    adult_df = pd.DataFrame(data=np.hstack([X_resampled, y_resampled.values.reshape((y_resampled.__len__(), 1))]),
                            columns=['age', 'sex', 'workclass', 'education', 'marital-status', 'hours', 'income'])

    adult_df.to_csv(os.path.join(src_path, '../data/adult_binary.csv'), index=False)


def preprocessing():
    df = pd.read_csv(os.path.join(src_path, '../data/adult_binary.csv'))
    ...
    train.to_csv(os.path.join(src_path, '../data/adult_binary_train.csv'), index=False)
    test.to_csv(os.path.join(src_path, '../data/adult_binary_test.csv'), index=False)
